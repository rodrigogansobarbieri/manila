...
class MigrationNFSTest(base.BaseSharesAdminTest):
    ...
    @test.attr(type=["gate", ])
    def test_migration_completion_empty_v2_11(self):

        share, dest_pool = self._setup_migration()

        old_exports = self.shares_v2_client.list_share_export_locations(
            share['id'], version='2.11')
        ...
        share = self.migrate_share(
            share['id'], dest_pool, version='2.11', notify=False,
            wait_for_status='migration_copying_completed')

        self._validate_migration_successful(dest_pool, share,
                                            old_exports, '2.11', notify=False)

        share = self.migration_complete(share['id'], dest_pool, version='2.11')
    ...
    def _validate_migration_successful(self, dest_pool, share,
        ...
        if utils.is_microversion_lt(version, '2.11'):
        ...
        # Share not migrated yet
            ...
            self.assertEqual('migration_copying_completed',
                             share['task_state'])
