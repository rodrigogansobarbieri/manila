...
class ManageNFSShareTest(base.BaseSharesAdminTest):
    ...
    def test_manage_invalid(self):
        ...
        try:
            ...
            # If manage share somehow succeeded with incorrect values, then
            # delete the share, thus removing database entry and
            # backend resource.
            _delete_share(share['id'])
            raise
        # Delete database entry of share that failed managing.
        _delete_share(share['id'])
        ...
        try:
            ...
            # If manage share somehow failed with correct values, then
            # delete the share, thus removing database entry.
            _delete_share(share['id'])
            raise
        # Delete database entry and backend resource of share used for
        # this test.
        _delete_share(share['id'])
