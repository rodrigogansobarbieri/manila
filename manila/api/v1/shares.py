...
class ShareMixin(object):
    ...
    def _complete_migration(self, req, id, body):
        ...
        try:
            ...
            msg = _("Share %s not found.") % id
            raise exc.HTTPNotFound(explanation=msg)
        self.share_api.complete_migration(context, share)
        return webob.Response(status_int=202)

    def _cancel_migration(self, req, id, body):
        ...
        try:
            ...
            msg = _("Share %s not found.") % id
            raise exc.HTTPNotFound(explanation=msg)
        self.share_api.cancel_migration(context, share)
        return webob.Response(status_int=202)

    def _get_migration_progress(self, req, id, body):
        ...
        try:
            ...
            msg = _("Share %s not found.") % id
            raise exc.HTTPNotFound(explanation=msg)
        result = self.share_api.get_migration_progress(context, share)
    ...
    def _reset_task_state(self, req, id, body):
        ...
        if update['task_state'] not in constants.TASK_STATE_STATUSES:
