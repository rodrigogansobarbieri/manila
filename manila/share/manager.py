...
class ShareManager(manager.SchedulerDependentManager):
    ...
    def _provide_share_server_for_share(self, context, share_network_id,
        ...
        def _provide_share_server_for_share():
            ...
            if create_in_backend:
                compatible_share_server, share_instance_ref = (
                    self.create_share_server_in_backend(
                        context, compatible_share_server, share_instance_ref))

            return compatible_share_server, share_instance_ref

        return _provide_share_server_for_share()

    def create_share_server_in_backend(
            self, context, share_server, share_instance):
        ...
        return share_server, share_instance

    def create_remote_share_server(
            self, context, share_instance_id, share_server_id):
        ...
        self.create_share_server_in_backend(
            context, share_server, share_instance)
    ...
    def _migration_start_driver(self, context, share_ref, share_instance,
        ...
        dest_driver_migration_info = rpcapi.migration_get_driver_info(
            context, share_instance['id'], dest_host['host'])
        ...
        service_list = self.db.service_get_all_by_host(context, host)
        share_api = api.API()

        request_spec, migrating_instance = (
            share_api._create_share_instance_and_get_request_spec(
                context, share_ref,
                service_list[0]['availability_zone_id'],
        ...
        try:
            ...
            moved, export_locations = self.driver.migration_start(
                context, share_instance, dest_host, dest_driver_migration_info,
                migrating_instance, notify, rules, share_server,
                dest_share_server)
            ...
            if moved and notify:
                ...
                if self.access._check_needs_refresh(
                        context, rules, migrating_instance):
                    self.access.update_access_rules(
                        context, migrating_instance['id'],
    ...
    def _migration_complete_driver(
        ...
        dest_driver_migration_info = rpcapi.migration_get_driver_info(
            context, share_instance['id'], dest_host)
        ...
        if self.access._check_needs_refresh(
            ...
            self.access.update_access_rules(
                context, new_share_instance['id'], share_server=share_server)
    ...
    @utils.require_driver_initialized
    def migration_cancel(self, context, share_id, share_instance_id,
    ...
    @utils.require_driver_initialized
    def migration_get_progress(self, context, share_id, share_instance_id,
                               migrating_instance_id):
