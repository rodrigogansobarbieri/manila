...
class NFSHelperTestCase(test.TestCase):
    ...
    def test_update_access_add_rule_no_ip(self):
        add_rules = [test_generic.get_fake_access_rule(
    ...
    @ddt.data(const.ACCESS_LEVEL_RW, const.ACCESS_LEVEL_RO)
    def test_update_access_add_existing_rule(self, level):
        add_rules = [test_generic.get_fake_access_rule('2.2.2.2', level), ]
        local_path = os.path.join(CONF.share_mount_path, self.share_name)
        exec_result = ' '.join([local_path, '2.2.2.2'])
...
class CIFSHelperIPAccessTestCase(test.TestCase):
    ...
    def test_update_access_add_existing_rule(self):
        add_rules = [test_generic.get_fake_access_rule(
            '2.2.2.2', const.ACCESS_LEVEL_RW), ]
        self.mock_object(self._helper, '_get_allow_hosts',
                         mock.Mock(return_value=['2.2.2.2', ]))
    ...
    def test_update_access_recovery_mode(self):
        access_rules = [test_generic.get_fake_access_rule(
            '1.1.1.1', const.ACCESS_LEVEL_RW), ]
        self.mock_object(self._helper, '_set_allow_hosts')
    ...
    def test_get_allow_hosts(self):
        # TODO(ganso): get an example output to use here
        self._helper._get_allow_hosts(self.server_details, self.share_name)
...
class CIFSHelperUserAccessTestCase(test.TestCase):
    ...
    def test_update_access(self):
        self.mock_object(self._helper, '_get_valid_users',
                         mock.Mock(side_effect=[['user3'], ['user4']]))
        add_access = [test_generic.get_fake_access_rule(
            'user1', 'rw', access_type='user'),
            test_generic.get_fake_access_rule(
                'user2', 'ro', access_type='user')]
        delete_access = [test_generic.get_fake_access_rule(
            'user3', 'rw', access_type='user'),
            test_generic.get_fake_access_rule(
                'user4', 'ro', access_type='user')]
        self._helper.update_access(self.server_details, self.share_name, [],
                                   add_access, delete_access)

    def test_update_access_exception_exists(self):
        self.mock_object(self._helper, '_get_valid_users',
                         mock.Mock(side_effect=[['user1'], []]))
        add_access = [test_generic.get_fake_access_rule(
            'user1', 'rw', access_type='user')]
        self.assertRaises(exception.ShareAccessExists,
                          self._helper.update_access, self.server_details,
                          self.share_name, [], add_access, None)

    def test_update_access_exception_type(self):
        self.mock_object(self._helper, '_get_valid_users',
                         mock.Mock(side_effect=[[], ['user2']]))
        add_access = [test_generic.get_fake_access_rule(
            'user1', 'rw', access_type='ip')]
    ...
    @ddt.data({'access_level': const.ACCESS_LEVEL_RW, 'force': False},
              {'access_level': const.ACCESS_LEVEL_RO, 'force': True})
    @ddt.unpack
    def test_get_valid_users(self, access_level, force):
        users = ("\"manila-user\" \"user1\" \"user2\"", None)
        self.mock_object(self._helper, '_ssh_exec',
                         mock.Mock(return_value=users))
        result = self._helper._get_valid_users(
            self.server_details, self.share_name, access_level, force=force)
        self.assertEqual(['manila-user', 'user1', 'user2'], result)
        if access_level == const.ACCESS_LEVEL_RW:
            expected_param = 'valid users'
        else:
            expected_param = 'read list'
        self._helper._ssh_exec.assert_called_once_with(
            self.server_details, ['sudo', 'net', 'conf', 'getparm',
                                  self.share_name, expected_param])
